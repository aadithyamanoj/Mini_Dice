...
module tb_chip_top;
  ...
  assign PAD[0]  = clk_i;                       // clk_i / io_master_clk_i
  assign PAD[1]  = rst_i;                        // rst_i
  // PAD[2]  = token_clk_i      — driven from ep_dn_token_r below
  assign PAD[3]  = dut_upstream_io_link_reset;   // upstream_io_link_reset_i
  assign PAD[4]  = dut_async_token_reset;        // async_token_reset_i
  assign PAD[5]  = dut_downstream_io_link_reset; // downstream_io_link_reset_i
  // PAD[6:15]  — chip upstream outputs (read below)
  // PAD[16:17] — driven from ep upstream outputs below
  // PAD[18:25] — driven from ep upstream data below
  // PAD[26]    — chip downstream token output (read below)
  // PAD[27:47] — spare/unused: drive to 0
  assign PAD[31] = 1'b0;
  assign PAD[32] = 1'b0;
  assign PAD[33] = 1'b0;
  assign PAD[34] = 1'b0;
  ...
  wire        dut_up_clk_r   = PAD[6];
  wire        dut_up_valid_r = PAD[7];
  wire [CW-1:0] dut_up_data_r;
  genvar gi;
  generate
    for (gi = 0; gi < CW; gi++) begin : gen_up_data
      assign dut_up_data_r[gi] = PAD[8 + gi];
    end
  endgenerate
  wire dut_dn_token_r = PAD[26];
  ...
  // EP upstream outputs → chip downstream inputs (PAD[16:17, 18:25])
  wire        ep_up_clk_r;
  wire        ep_up_valid_r;
  wire [CW-1:0] ep_up_data_r;
  wire          ep_dn_token_r;

  assign PAD[16] = ep_up_clk_r;
  assign PAD[17] = ep_up_valid_r;
  generate
    for (gi = 0; gi < CW; gi++) begin : gen_dn_data
      assign PAD[18 + gi] = ep_up_data_r[gi];
    end
  endgenerate
  assign PAD[2] = ep_dn_token_r;  // token_clk_i
  ...
  // --------------------------------------------------------------------------
  // Completion / timeout monitor (hierarchy goes through u_dut.u_mini_dice)
  // --------------------------------------------------------------------------
  ...
  always_ff @(posedge clk_i) begin
    if (rst_i) begin
      ...
      if (u_dut.u_mini_dice.u_csr.complete_sticky_r && complete_seen_cycle == 0)
  ...
  // --------------------------------------------------------------------------
  // Debug probes (same signals, deep path now includes u_mini_dice)
  // --------------------------------------------------------------------------
  ...
  always_ff @(posedge clk_i) if (!rst_i) begin
    if (u_dut.u_mini_dice.mfetch_req.ar_valid && u_dut.u_mini_dice.mfetch_resp.ar_ready)
      $display("[HIER][DC] t=%0t mfetch AR addr=0x%04x len=%0d", $time,
               u_dut.u_mini_dice.mfetch_req.ar.addr, u_dut.u_mini_dice.mfetch_req.ar.len);
    if (u_dut.u_mini_dice.bsfetch_req.ar_valid && u_dut.u_mini_dice.bsfetch_resp.ar_ready)
      $display("[HIER][DC] t=%0t bsfetch AR addr=0x%04x len=%0d", $time,
               u_dut.u_mini_dice.bsfetch_req.ar.addr, u_dut.u_mini_dice.bsfetch_req.ar.len);
    if (u_dut.u_mini_dice.dfetch_arvalid && u_dut.u_mini_dice.dfetch_arready)
      $display("[HIER][DC] t=%0t dfetch AR addr=0x%04x", $time, u_dut.u_mini_dice.dfetch_araddr);
    if (u_dut.u_mini_dice.dfetch_awvalid && u_dut.u_mini_dice.dfetch_awready)
      $display("[HIER][DC] t=%0t dfetch AW addr=0x%04x", $time, u_dut.u_mini_dice.dfetch_awaddr);
    if (u_dut.u_mini_dice.u_io_top.xbar_mem_req.ar_valid &&
        u_dut.u_mini_dice.u_io_top.xbar_mem_resp.ar_ready)
      $display("[HIER][XB] t=%0t xbar_mem AR addr=0x%04x len=%0d", $time,
               u_dut.u_mini_dice.u_io_top.xbar_mem_req.ar.addr,
               u_dut.u_mini_dice.u_io_top.xbar_mem_req.ar.len);
  end
  ...
  always_ff @(posedge clk_i) if (!rst_i) begin
    ...
    if ((hb_count % 2000) == 0)
      $display("[HB] t=%0t beat=%0d  ep_tx_rv=%b rrdy=%b | chip.rx_rv=%b rrdy=%b",
               ...
               u_dut.u_mini_dice.u_io_top.rx_rvalid,
               u_dut.u_mini_dice.u_io_top.xbar_mem_req.r_ready);
  end

  int csr_dump_once;
  always_ff @(posedge clk_i) if (!rst_i) begin
    if (u_dut.u_mini_dice.u_csr.start_o && csr_dump_once == 0) begin
      csr_dump_once <= 1;
      $display("[CSR] t=%0t start_o=1 start_pc=0x%04x csrX0..7={%04x %04x %04x %04x %04x %04x %04x %04x}",
               $time, u_dut.u_mini_dice.u_csr.start_pc_o,
               u_dut.u_mini_dice.u_csr.csrX_r[0], u_dut.u_mini_dice.u_csr.csrX_r[1],
               u_dut.u_mini_dice.u_csr.csrX_r[2], u_dut.u_mini_dice.u_csr.csrX_r[3],
               u_dut.u_mini_dice.u_csr.csrX_r[4], u_dut.u_mini_dice.u_csr.csrX_r[5],
               u_dut.u_mini_dice.u_csr.csrX_r[6], u_dut.u_mini_dice.u_csr.csrX_r[7]);
    end
    if (u_dut.u_mini_dice.u_cta_if.dispatch_valid && u_dut.u_mini_dice.u_cta_if.dispatch_ready)
      $display("[CTA] t=%0t dispatch handshake start_pc=0x%04x", $time,
               u_dut.u_mini_dice.u_cta_if.dispatch_data.kernel_desc.start_pc);
  end
