#!/usr/bin/env python3
"""Generate the SV test-data block for a full_mul_array test on cgra-nopred.

Reads:
  - tb/test_vectors/full_mul_array_test_vector.json   (CTA, runtime, expected,
...
Usage:
    python3 gen_test_data.py > /tmp/test_data.svh
"""
...
# Stage order for naming the .bin files (must match the order of pgraph_metadata)
STAGES = (
    "load_mul_array_a",
    "load_mul_array_b",
    "mul_array",
...
def emit_meta_block(stage_idx: int, meta: dict) -> str:
...
def emit_bitstream_block(stage_idx: int, bitstream_addr: int) -> str:
...
def main() -> None:
    tv  = json.loads((TVEC / "full_mul_array_test_vector.json").read_text())
    ...
    out.append("// Source: tb/test_vectors/full_mul_array_test_vector.json")
    out.append("//         dora/.../build_nopred/mini_dice_<kernel>.bin")
    out.append("// ============================================================")
    out.append("")

    for i, p in enumerate(pmeta):
        out.append(emit_meta_block(i, p["meta"]))

    for i, p in enumerate(pmeta):
        out.append(emit_bitstream_block(i, p["meta"]["bitstream_addr"]))
