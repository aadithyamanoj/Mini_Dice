...
// produces PR0 = (tid != 0); tid 0 takes the ADD path, tids 1..31 take MUL.
...
class dice_core_simple_branching_test extends dice_core_full_mul_array_test;
  ...
  // start_pc and thread_count are identical (0x1000 / 32), so the parent's
  // run_body dispatches the same CTA — only the 7-eblock metadata graph
  // and the 128 expected stores differ.
