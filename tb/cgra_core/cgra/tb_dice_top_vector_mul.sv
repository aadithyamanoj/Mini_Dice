...
module tb_dice_top_vector_mul;
  ...
  localparam int BITSTREAM_BYTES = 212;
  localparam int BITSTREAM_SIZE_BITS = 1690;
  ...
  task automatic load_bin_bitstream();
    ...
    begin
      ...
      if (bytes_read < BITSTREAM_BYTES) begin
        $fatal(1, "Bitstream file too short: expected at least %0d bytes, got %0d",
               BITSTREAM_BYTES, bytes_read);
      end

      $display("[TB] Programming bitstream from %s (%0d bytes, %0d bits)", bitstream_file,
               bytes_read, BITSTREAM_SIZE_BITS);
  ...
  task automatic test_mul_array_randomized();
    ...
    begin
      ...
      for (case_idx = 0; case_idx < NUM_RANDOM_CASES; case_idx++) begin
        ...
        apply_mul_array_inputs(ext_data_i, ext_pred_i, a_values, b_values);
        repeat (FUNCTIONAL_SETTLE) @(posedge clk_i);
        check_mul_outputs($sformatf("randomized case %0d", case_idx), ext_data_o, a_values,
                          b_values, expected_values);
      end
