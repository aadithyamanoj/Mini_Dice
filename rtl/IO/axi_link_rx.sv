module axi_link_rx
  ...
  //   WRITE_REQ : [31:30] opcode, [29:16] reserved (always 0),            [15:0] address
  //               Writes are always single beat — there is no wire len
  //               field; the parser unconditionally expects exactly one W
  //               payload flit and replays AW with awlen_o=0.
  ...
  // this field — writes are single-beat only.
  //
  ...
  //   WRITE_REQ  -> AW (awlen=0) + 1 W beat (single-beat only)
  //   FETCH_REQ  -> AR burst
      ...
      RX_IDLE: begin
        if (link_fifo_v_lo) begin
          unique case (hdr_opcode)
            OP_WRITE_REQ: begin
              // Header carries {opcode,addr}; the len field is reserved
              // and ignored because writes are single-beat only. Publish
              // the AW descriptor with beats=1 and stream exactly one W
              // payload flit next.
              if (wr_desc_push_ready_lo) begin
                link_fifo_yumi_li    = 1'b1;
                wr_desc_push_v_li    = 1'b1;
                wr_desc_push_data_li = {hdr_addr, beat_count_width_lp'(1)};
