module axi_link_tx
  ...
  //   WRITE_REQ : [31:30] opcode, [29:16] reserved (hardcoded 0),                   [15:0] address
  //               Writes are restricted to a single beat. The on-wire len
  //               field is hardcoded to 0 (AXI-style: 0 = 1 beat); upstream
  //               must drive awlen_i=0 or AW will be rejected.
  ...
  // burst length max is 256, so 8 bits suffice. WRITE_REQ does not use
  // this field — it is fixed at 0 to enforce single-beat writes.
  //
  ...
  //   per accepted write (BRESP=OKAY) so the AXI master sees write
  //   completion locally; per-write error reporting is therefore lost.
  //   Writes are single-beat only: AW is rejected unless awlen_i=0, so a
  //   "burst" on this link is always exactly one W flit.
  //
  // Transport:
  //   WRITE_REQ  = header(opcode,addr)         + 1 W beat (single-beat only)
  ...
  // Writes must be single beat (awlen_i=0) — the WRITE_REQ header has no
  // wire length field and the link transports exactly one W flit per
  // accepted AW.
  ...
  always_comb begin
    ...
    unique case (state_r)
      TX_HEADER: begin
        link_tx_v_o = 1'b1;
        // WRITE_REQ has no wire length field — writes are single-beat only,
        // so [29:16] is hardcoded to zero. FETCH encodes len as AXI-style
        // (beats - 1) in 8 bits at [23:16]. LOAD keeps the full 13-bit
        // aruser at [28:16]; the opcode disambiguates.
