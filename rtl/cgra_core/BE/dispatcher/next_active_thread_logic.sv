...
#(
  ...
  logic [5:0] encoded_pos_rm;  // reverse_mapper expects a 6-bit encoded position
  logic [7:0] active_mask_index_rm;  // reverse_mapper returns an 8-bit index
  ...
  reverse_mapper #(
      ...
      .encoded_pos(encoded_pos_rm),
      .unrolling_factor(unrolling_factor),
      .valid_in(pe_valid),
      .active_mask_index(active_mask_index_rm),
      .valid_out(rm_valid)
  );

  assign encoded_pos_rm    = {{(6-LANE_WIDTH){1'b0}}, encoded_pos};
  assign active_mask_index = active_mask_index_rm[$clog2(CHUNK_SIZE)-1:0];
