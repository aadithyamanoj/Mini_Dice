...
(
    ...
    assign full_reg_bitmap_o = {pred_bitmap, const_bitmap, gpr_bitmap}; // Output the full register bitmap to the RF controller
    ...
    logic [NUM_LANES-1:0] sb_rsv_valid_per_sb [NUM_SCOREBOARDS];      // [scoreboard][lane] - for reserve operations
    ...
    assign check_tid[0]   = thread_next_tid_0[SCOREBOARD_TID_WIDTH-1:0];
    ...
    always_comb begin
        ...
        if (sb_rsv_valid[0])
            sb_rsv_valid_per_sb[0][0] = 1'b1;  // NUM_SCOREBOARDS=1, always route to scoreboard 0
    end
    ...
    constant_scoreboard #(.NUM_CONSTANT_REGS(DICE_NUM_CONST)) const_sb (
        .clk(clk_i),
        .rst(rst),
        ...
        .wb_const_bitmap(ld_dest_regs_bitmap[(DICE_NUM_REGS + DICE_NUM_CONST - 1):DICE_NUM_REGS]),
        .wb_valid(wb_valid && |ld_dest_regs_bitmap[(DICE_NUM_REGS + DICE_NUM_CONST - 1):DICE_NUM_REGS]),
    ...
    always_comb begin
        ...
        for (int j = 0; j < NUM_LANES; j++) begin
            ready_fifo_push_en[j] = is_checking_collision &&
