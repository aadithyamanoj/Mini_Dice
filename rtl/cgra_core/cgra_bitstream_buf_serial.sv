...
module cgra_bitstream_buf_serial
  import dice_pkg::*;
#(
    parameter int PROG_BITSTREAM_BITS_P = 1666,
...
(
  ...
  localparam int reset_ctr_width_lp = (PROG_RESET_CYCLES_P > 1)
                                      ? $clog2(PROG_RESET_CYCLES_P)
                                      : 1;
  localparam int flush_ctr_width_lp = (PROG_FLUSH_CYCLES_P > 1)
                                      ? $clog2(PROG_FLUSH_CYCLES_P)
                                      : 1;
  ...
  always_comb begin
    ...
    unique case (state_r)
      ...
      e_prog_shift: begin
        if (serializer_yumi_li
            && (bit_ctr_r == bit_ctr_width_lp'(PROG_BITSTREAM_BITS_P-1))) begin
